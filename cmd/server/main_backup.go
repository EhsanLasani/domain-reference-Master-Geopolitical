...
func main() {
	...
	_, err := tracing.NewTracer("reference-master-geopolitical")
	if err != nil {
		log.Printf("Warning: Failed to initialize tracer: %v", err)
	}
	_, err = metrics.NewMetrics("reference-master-geopolitical")
	if err != nil {
	...
	rateLimiter := middleware.NewRateLimiter(1000, 100) // 1000 requests per minute, burst of 100
	...
	// Redirect root to Business Central interface
